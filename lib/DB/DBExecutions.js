...
export const getRestaurants = async () => {
    ...
    const result = await resturants.find({}, { projection: { name: 1, images: 1, description: 1, rating: 1, address: 1 } }).toArray();
...
export const getRestaurantsReview = async (id) => {
    ...
    const result = await reviews.find({ restaurant_id: { $ref: collection, $id: ObjectId(id) } }).toArray();
